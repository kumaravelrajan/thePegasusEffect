...
with open(f"/mnt/c/Users/kumar/Desktop/TUM/Seminar/pegasus_cybercrime_seminar_latest/beautifulsoup-kumar/json-files/Publishing-Houses/Edwy-Plenel.json", "r") as f:
        jsonData = json.load(f)

        for jsonElement in jsonData:
            html_text = requests.get(jsonElement['url'])
            soup = BeautifulSoup(html_text.text, 'lxml')
            #print(soup.find('div', class_='introduction').findAll('snap'))
            introText =  soup.find('div', class_='introduction').findAll('span')
            articleContentList = soup.findAll('div', class_="content-article")
